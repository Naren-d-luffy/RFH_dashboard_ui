...
//       setTableData(eventsData.map(event => ({ ...event, key: event._id })));
//     }
//   }, [eventsData]);
...
//       // Ensure indexes exist before calling arrayMove
...
//             <SortableContext items={tableData.map((item) => item._id)} strategy={verticalListSortingStrategy}>
//               <Table
//                 columns={columns}
...
const TableEventsList = () => {
  ...
  const handleDeleteEvent = (_id) => {
    message.success("Deleted successfully event");
    dispatch(deleteEvent(_id));
  };
  ...
  const onDragEnd = (event) => {
    ...
    setTableData((prev) => {
      ...
      // If the drag was within valid indexes, reorder the data
      if (oldIndex === -1 || newIndex === -1) return prev;
      const newItems = arrayMove(prev, oldIndex, newIndex);
      // Update the backend or further state management if needed
      return [...newItems];
    });
  };
  ...
  return (
    <div className="container mt-1">
      ...
      <AddEventsList open={isModalOpen} handleCancel={handleCancel} />
      <EditEventsList open={isEditModalOpen} handleCancel={handleEditCancel} eventsData={selectedEvent} />
      <ViewEventList open={isViewModalOpen} handleCancel={handleViewCancel} eventsData={selectedEvent} />
    </div>
  );
};
